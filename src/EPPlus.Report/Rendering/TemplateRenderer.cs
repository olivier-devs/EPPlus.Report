...
namespace EPPlus.Report.Rendering
{
    public class TemplateRenderer : ITemplateRenderer
    {
        ...
        public TemplateRenderer(IExpressionEvaluator evaluator, TemplateErrors renderingErrors = null, RowOperationTracker tracker = null, TemplateErrors warnings = null)
        {
        ...
        private object TryEvaluate(string expression, object context, ExcelWorksheet worksheet, int row, int column, string cellAddress, string functionName = null)
        {
            try
            {
            ...
            catch (Evaluation.PropertyNotFoundException ex)
            {
                if (_warnings != null)
                {
            ...
            catch (Exception ex) when (ex is ArgumentException || ex is NullReferenceException || ex is InvalidOperationException)
            {
                if (_renderingErrors == null)
                    throw;
        ...
        private object ApplyFunction(string functionName, object value, ExcelWorksheet worksheet, int row, int column, string cellAddress)
        {
            try
            {
                if (_evaluator is ExpressionEvaluator ee)
        ...
        private int RenderNode(TemplateNode node, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            switch (node)
            {
                case ExpressionNode exprNode:
                {
                    ...
                    if (context.Variables != null && context.Variables.TryGetValue(exprNode.ExpressionPath, out var varValue))
                    {
                        value = varValue;
                    }
                    else if (context.IsNamedRangeLoop)
                    {
                        value = ResolveNamedRangeExpression(exprNode.ExpressionPath, context, worksheet, targetRow, targetCol, cellAddress);
                    }

                    if (value == null)
                    {
                        value = TryEvaluate(exprNode.ExpressionPath, context.Current, worksheet, targetRow, targetCol, cellAddress, exprNode.FunctionName);
                    }
        ...
        private int RenderLoop(LoopNode loopNode, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            ...
            {
                collection = TryEvaluate(loopNode.CollectionName, context.Current, worksheet, blockStartRow, blockStartCol, blockStartAddress) as IEnumerable;
            }
            ...
            ReconcileConditionalFormatting(loopNode.ConditionalFormattingRules, loopNode.Row + rowOffset, loopNode.EndRow + rowOffset, loopNode.Row + rowOffset, loopNode.EndRow + currentOffset, worksheet);
            return currentOffset;
        }

        private int RenderNamedRangeLoop(NamedRangeLoopNode loopNode, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            IEnumerable collection = null;
            var blockStartRow = loopNode.Row + rowOffset;
            var blockStartCol = loopNode.Column;
            ...
            if (dataRowCount < 0) dataRowCount = 0;
        ...
        private int RenderNamedRangeGroup(NamedRangeLoopNode loopNode, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            IEnumerable collection = null;
            var blockStartRow = loopNode.Row + rowOffset;
            var blockStartCol = loopNode.Column;
            ...
            if (context.Variables != null && context.Variables.TryGetValue(loopNode.CollectionName, out var varValue))
            {
                collection = varValue as System.Collections.IEnumerable;
            }
            else if (context.Current != null)
            {
                collection = TryEvaluate(loopNode.CollectionName, context.Current, worksheet, blockStartRow, blockStartCol, blockStartAddress) as System.Collections.IEnumerable;
            }
            var items = collection?.Cast<object>().ToList() ?? new List<object>();
        ...
        private int RenderAggregation(AggregationNode aggNode, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            object result = null;
            ...
            if (aggNode.AggregationType == "sum")
            {
                ...
                if (collection != null)
                {
                    ...
                    {
                        var val = TryEvaluate(aggNode.PropertyName, item, worksheet, aggNode.Row + rowOffset, aggNode.Column, worksheet.Cells[aggNode.Row + rowOffset, aggNode.Column].Address);
                        if (val != null)
                        {
                            sum += System.Convert.ToDecimal(val);
                        }
                    }
                    result = sum;
                }
            ...
            else if (aggNode.AggregationType == "count")
            {
                ...
                {
                    var val = TryEvaluate(aggNode.PropertyName, context.Current, worksheet, aggNode.Row + rowOffset, aggNode.Column, worksheet.Cells[aggNode.Row + rowOffset, aggNode.Column].Address);
        ...
        private int RenderIf(IfNode ifNode, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            ...
            var conditionValue = TryEvaluate(ifNode.ConditionExpression, context.Current, worksheet, conditionRow, conditionCol, conditionAddress);
            bool isTrue = conditionValue is bool b && b;
            ...
            if (isTrue)
            {
                ...
                ReconcileConditionalFormatting(ifNode.ConditionalFormattingRules, ifNode.Row + rowOffset, ifNode.EndRow + rowOffset, ifNode.Row + rowOffset, ifNode.EndRow + childOffset, worksheet);
                return childOffset;
            }
            else
            {
        ...
        private int RenderGroup(GroupNode groupNode, RenderContext context, ExcelWorksheet worksheet, int rowOffset)
        {
            ...
            {
                collection = TryEvaluate(groupNode.CollectionName, context.Current, worksheet, blockStartRow, blockStartCol, blockStartAddress) as System.Collections.IEnumerable;
            }
            var items = collection?.Cast<object>().ToList() ?? new List<object>();
            ...
            // Sort and group
            var groups = GroupRenderer.SortAndGroup(items, groupNode.GroupByPaths, _evaluator, groupNode.Options.Descending);
            ...
            for (int g = 0; g < groups.Count; g++)
            {
                ...
                for (int i = 0; i < group.Items.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (dataTemplateHeight > 0)
                        {
                            ...
                            worksheet.InsertRow(insertAtRow, dataTemplateHeight, blockStartRow + 1 + currentOffset - rowOffset);
                        }
                    ...
                    && groupLastDataRow > groupFirstDataRow)
                {
                    ...
                    if (groupNode.Options.MergeLabels == MergeMode.Merge1 || groupNode.Options.MergeLabels == MergeMode.Merge2)
                    {
                        for (int r = groupFirstDataRow + 1; r <= groupLastDataRow; r++)
                        {
            ...
            ReconcileConditionalFormatting(groupNode.ConditionalFormattingRules, groupNode.Row + rowOffset, groupNode.EndRow + rowOffset, groupNode.Row + rowOffset, groupNode.EndRow + currentOffset, worksheet);
            return currentOffset;
        }

        private object ResolveNamedRangeExpression(string expression, RenderContext context, ExcelWorksheet worksheet, int row, int column, string cellAddress)
        {
            if (expression == "item")
        ...
        private void ReconcileConditionalFormatting(List<ConditionalFormattingRule> rules, int originalStartRow, int originalEndRow, int finalStartRow, int finalEndRow, ExcelWorksheet worksheet)
        {
            if (rules == null || rules.Count == 0)
                return;

            // Remove old rules that intersected the original block range
            var rulesToRemove = new List<OfficeOpenXml.ConditionalFormatting.Contracts.IExcelConditionalFormattingRule>();
            ...
            {
                var newAddress = worksheet.Cells[finalStartRow, 1, finalEndRow, worksheet.Dimension?.End.Column ?? 1].Address;
        ...
        private void ApplyServiceTag(ServiceTag tag, int firstDataRow, int lastDataRow, ExcelWorksheet worksheet, int row)
        {
            ...
            switch (tag.TagName)
            {
                ...
                case "count":
                {
                    ...
                    {
                        var val = worksheet.Cells[r, tag.Column].Value;
                        if (val != null && !string.IsNullOrWhiteSpace(val.ToString())) count++;
                    }
                    cell.Value = count;
                    break;
                }
